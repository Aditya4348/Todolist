<div class="max-w-lg mx-auto p-6 bg-white shadow-md rounded-lg mt-10">
    ...
    @if (session()->has('pesan'))
        <div x-data="{ show:true}" x-show="show" x-init="setTimeout(() => show = false,3000)" class="bg-green-500 text-white p-4 rounded-md mb-4 duration-300">
            {{ session('pesan') }}
        </div>
    @endif
    ...
    <div class="mt-8">
        ...
        <ul class="space-y-2">
            ...
            @if (session()->has('pesanHapus'))
                <div x-data="{ show:true}" x-show="show" x-init="setTimeout(() => show = false,3000)" class="bg-green-500 text-white p-4 rounded-md mb-4 ">
                    {{ session('pesanHapus') }}
                </div>
            @endif
        </ul>
